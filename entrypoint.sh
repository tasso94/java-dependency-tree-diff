...
if [[ $(git diff origin/master HEAD --name-only | grep pom.xml$ | wc -c) -ne 0 ]]; then
    ...
    mvn -T 8C org.codehaus.mojo:license-maven-plugin:2.0.0:aggregate-download-licenses -Dlicense.excludedScopes=test,compile,runtime,provided -Dlicense.excludedGroups="org\.camunda.*"
    find . -name 'licenses.xml' -exec rsync -R \{\} /pr \;

    git checkout -f origin/master
    mvn -T 8C org.codehaus.mojo:license-maven-plugin:2.0.0:aggregate-download-licenses -Dlicense.excludedScopes=test,compile,runtime,provided -Dlicense.excludedGroups="org\.camunda.*"
    find . -name 'licenses.xml' -exec rsync -R \{\} /master \;
